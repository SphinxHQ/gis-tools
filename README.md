...
- **多格式数据导入**: 支持 GeoJSON、WKT、WKB、Shapefile、DXF、EXF、电子报盘(交换格式) 等格式
- **坐标系识别与转换**: 自动识别常用坐标系，支持投影↔地理坐标互转、同系投影间转换
- **地图可视化**: 基于 OpenLayers 的地图展示与交互，支持投影坐标系直接渲染
- **绘图工具**: 支持在地图上绘制点、线、面，可选择投影坐标系
- **数据编辑**: 结构化树+JSON编辑器，支持直接编辑要素几何
- **数据导出**: 支持 GeoJSON、WKT、Shapefile、电子报盘(交换格式) 导出
- **几何校验**: 自相交、环方向、孔洞位置、重复顶点等检查与自动修复
- **暗色模式**: 支持亮色/暗色主题切换
